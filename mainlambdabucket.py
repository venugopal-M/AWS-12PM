...
def lambda_handler(event, context):
 ...
 #copy from s3 to s3
        if ((extension==".png") or (extension==".jpeg") or (extension==".jpg")):
            s3.copy_object(Bucket="imgbkt-1", Key=key, CopySource=copy_source)
        if ((extension==".docx") or (extension==".doc") or (extension==".pdf")):
            s3.copy_object(Bucket="pdfbkt-1", Key=key, CopySource=copy_source)
